  ...
  group('ParameterRegistry', () {
    ...
    test('canonicalize resolves aliases and ignores unknown parameters', () {
      final canonical = registry.canonicalize(<String, double>{
        ...
        'portamento': 3.2,
        'pitch_bend_range': 36,
        'pitch_bend_amount': -2.0,
        'unknown': 123,
      });

      expect(canonical.length, 13);
      ...
      expect(canonical['pitchWheelValue'], closeTo(-1.0, 1e-9));
    ...
    test('expandWithAliases mirrors canonical values to bridge aliases', () {
      ...
      expect(expanded['pitchWheelValue'], closeTo(-0.35, 1e-9));
      expect(expanded['pitchBendAmount'], closeTo(-0.35, 1e-9));
    });
    ...
    test('defaultValue exposes descriptor defaults', () {
      ...
      expect(registry.defaultValue('pitchWheelValue'), closeTo(0.0, 1e-9));
