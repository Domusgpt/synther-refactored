...
void main() {
  ...
  test('granular and wavetable metrics mirror parameter state', () async {
    ...
    backend.setParameter(SynthParameterId.macro1, 0.25);
    backend.setParameter(SynthParameterId.macro2, 0.9);
    backend.setParameter(SynthParameterId.macro3, 0.4);
    backend.setParameter(SynthParameterId.macro4, -0.2);
    ...
    expect(metrics['macro1'], closeTo(0.25, 1e-9));
    expect(metrics['macro2'], closeTo(0.9, 1e-9));
    expect(metrics['macro3'], closeTo(0.4, 1e-9));
    expect(metrics['macro4'], closeTo(0.0, 1e-9));
    expect(metrics['macroAverage'], closeTo((0.25 + 0.9 + 0.4 + 0.0) / 4, 1e-9));
    expect(
      metrics['performanceEnergy'],
      closeTo((0.75 + 0.6 + 1.0 + ((0.25 + 0.9 + 0.4 + 0.0) / 4)) / 4, 1e-9),
    );
