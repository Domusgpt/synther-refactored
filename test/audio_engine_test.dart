...
void main() {
  ...
  group('AudioEngine voice management', () {
    ...
    test(
      'setMaxPolyphony reduces voices immediately and notifies listeners',
      () async {
        final backend = BasicAudioBackend();
        final clock = _FakeClock();
    ...
    test(
      'voice stealing falls back to oldest voice when velocities match',
      () async {
        final backend = BasicAudioBackend();
        final clock = _FakeClock();
  ...
  group('Parameter bridge integration', () {
    test(
      'applies parameter updates from the bridge and clamps values',
      () async {
    ...
    test('publishes engine parameter changes back to the bridge', () async {
      ...
      final masterUpdate = updates.lastWhere(
        (event) => event.name == 'masterVolume',
      );
    ...
    test('lfo parameters clamp, sync and broadcast aliases', () async {
      ...
      expect(backend.getParameter(SynthParameterId.lfoRate), closeTo(30, 1e-9));
    ...
    test(
      'oscillator and fx parameters clamp, sync and broadcast aliases',
      () async {
        final bridge = ParameterBridge.instance;
        final backend = BasicAudioBackend();
        final engine = AudioEngine(backend: backend, parameterBridge: bridge);
        ...
        expect(
          backend.getParameter(SynthParameterId.oscillatorBlend),
          closeTo(0, 1e-9),
        );
    ...
    test(
      'performance controllers clamp and propagate via the bridge',
      () async {
        final bridge = ParameterBridge.instance;
        final backend = BasicAudioBackend();
        final engine = AudioEngine(backend: backend, parameterBridge: bridge);
  ...
  group('Arpeggiator controls', () {
    test(
      'applyArpeggiatorSettings updates backend and notifies once',
      () async {
        final backend = BasicAudioBackend();
        final engine = AudioEngine(backend: backend);
  ...
  group('Preset management', () {
    ...
    test('capturePreset includes modulation routes when present', () async {
      ...
      final routeKeys = preset.modulationRoutes
          .map((route) => route.key)
          .toSet();
    ...
    test(
      'applyPreset restores modulation matrix and visualizer metrics',
      () async {
        final bridge = ParameterBridge.instance;
        final backend = BasicAudioBackend();
        final engine = AudioEngine(backend: backend, parameterBridge: bridge);
        ...
        expect(
          visualizer['modDestination.distortiondrive'],
          closeTo(0.3, 1e-9),
        );
    ...
    test(
      'applyPreset updates backend, bridge and active preset snapshot',
      () async {
        final bridge = ParameterBridge.instance;
        final backend = BasicAudioBackend();
        final engine = AudioEngine(backend: backend, parameterBridge: bridge);
        ...
        expect(
          backend.getParameter(SynthParameterId.masterVolume),
          closeTo(0.33, 1e-9),
        );
        ...
        expect(
          engine.activePreset?.parameters['delayFeedback'],
          closeTo(0.52, 1e-9),
        );
    ...
    test('loadPreset parses nested alias-heavy preset data', () async {
      ...
      final presetData = <String, dynamic>{
        ...
        'category': 'ambient',
        'volume': 0.58,
        'filter': {'cutoff_hz': 28000, 'res': 0.72},
        'envelope': {
          'attack_time': 0.35,
          'decay': 0.4,
          'sustain': 0.6,
        ...
        'effects': {
          'reverb': 0.7,
          'delay': {'time': 0.55, 'feedback': 0.45},
        },
        'oscillator': {'blend': 0.32, 'detune_semitones': -8, 'spread': 1.4},
        'fx': {'drive': 1.6, 'chorus_rate': 14.0, 'chorus_mix': 1.5},
        'arpeggiator': {
          'enabled': true,
          'rate': 11.0,
          'gate': 0.35,
      ...
      expect(
        engine.activePreset?.metadata.category,
        SynthPresetCategory.ambient,
      );
      ...
      expect(
        engine.activePreset?.parameters['granularActive'],
        closeTo(1.0, 1e-9),
      );
      ...
      expect(
        engine.activePreset?.parameters['arpeggiatorRate'],
        closeTo(11.0, 1e-9),
      );
    ...
    test('manual preset loads clear setlist context', () async {
      ...
      await engine.loadSetlistEntry(setlistId: 'showcase', entryId: 'slot-1');
    ...
    test(
      'clearSetlistContext resets context without touching preset',
      () async {
        final backend = BasicAudioBackend();
        final engine = AudioEngine(backend: backend);
    ...
    test('setlist practice snapshot tracks timeline and looping', () async {
      final backend = BasicAudioBackend();
      final engine = AudioEngine(backend: backend);
      ...
      expect(
        snapshot.options.defaultEntryBeats,
        engine.setlistPracticeOptions.defaultEntryBeats,
      );

      final timeline = engine.setlistPracticeTimeline;
      expect(timeline.length, 3);
      expect(timeline.first.durationBeats, 16);
      expect(
        timeline[1].durationBeats,
        engine.setlistPracticeOptions.defaultEntryBeats,
      );
      expect(timeline[2].cueBeats, 12);

      await engine.advanceSetlistPractice(loadPreset: false, notify: false);
      expect(engine.setlistPracticeSnapshot?.current?.entry.id, 'slot-2');

      await engine.advanceSetlistPractice(loadPreset: false, notify: false);
      await engine.advanceSetlistPractice(loadPreset: false, notify: false);
      expect(engine.setlistPracticeSnapshot?.current?.entry.id, 'slot-1');
      expect(engine.setlistPracticeSnapshot?.loopCount, 1);
      ...
      final updatedTimeline = engine.setlistPracticeTimeline;
      expect(updatedTimeline[1].durationBeats, 24);
      expect(updatedTimeline[0].cueBeats, 2);

      engine.resetSetlistPractice(notify: false);
      expect(engine.setlistPracticeSnapshot?.loopCount, 0);
      expect(engine.setlistPracticeSnapshot?.current?.entry.id, 'slot-1');

      final advancedToEnd = await engine.advanceSetlistPractice(
        loadPreset: false,
        notify: false,
      );
      expect(advancedToEnd, isTrue);
      await engine.advanceSetlistPractice(loadPreset: false, notify: false);
      final loopBlocked = await engine.advanceSetlistPractice(
        loadPreset: false,
        notify: false,
      );
