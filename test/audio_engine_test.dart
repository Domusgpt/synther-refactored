...
import 'package:synther_holographic_pro/core/arpeggiator.dart';
import 'package:synther_holographic_pro/core/audio_engine.dart';
import 'package:synther_holographic_pro/core/audio_preset_library.dart';
...
void main() {
  ...
  group('AudioEngine macro controls', () {
    test('setMacroValue resolves assignments and notifies listeners', () async {
      final backend = BasicAudioBackend();
      final engine = AudioEngine(backend: backend);
      ...
      final cutoffRange = registry.descriptorFor('filterCutoff')!.range;
      final resonanceRange = registry.descriptorFor('filterResonance')!.range;

      final changed = await engine.setMacroValue('macro-brightness', 0.5);

      expect(changed, isTrue);
      expect(
        engine.filterCutoff,
        closeTo(cutoffRange.denormalize(0.5), 0.0001),
      );
      expect(
        engine.filterResonance,
        closeTo(resonanceRange.denormalize(0.4), 0.0001),
      );
    ...
    test('resetMacroValues returns macros to definition defaults', () async {
      final backend = BasicAudioBackend();
      final engine = AudioEngine(backend: backend);
      ...
      await engine.registerMacro(
        MacroDefinition(
          ...
          name: 'Glow',
          defaultValue: 0.35,
          assignments: const <MacroAssignment>[
            MacroAssignment(
      ...
      await engine.setMacroValue('macro-glow', 0.9);
      expect(engine.macroValue('macro-glow'), closeTo(0.9, 0.0001));

      await engine.resetMacroValues();

      expect(engine.macroValue('macro-glow'), closeTo(0.35, 0.0001));

      engine.dispose();
      backend.dispose();
    });

    test('capture and apply preset preserves macro definitions', () async {
      final backend = BasicAudioBackend();
      final engine = AudioEngine(backend: backend);
      ...
      await engine.registerMacro(
        MacroDefinition(
          id: 'performance-swell',
          name: 'Performance Swell',
          assignments: <MacroAssignment>[
            MacroAssignment(
              parameterId: 'oscillatorDetune',
              minNormalized: 0.4,
              maxNormalized: 1.0,
            ),
            MacroAssignment(
      ...
      await engine.setMacroValue('performance-swell', 1.0);

      final preset = engine.capturePreset(id: 'macro-test', name: 'Macro Test');
      expect(preset.macros, isNotEmpty);
      final macroSnapshot = preset.macros
          .firstWhere((macro) => macro.definition.id == 'performance-swell');
      expect(macroSnapshot.value, closeTo(1.0, 0.0001));

      final detuneRange =
          ParameterRegistry.instance.descriptorFor('oscillatorDetune')!.range;
      final driveRange =
          ParameterRegistry.instance.descriptorFor('distortionDrive')!.range;
      ...
      await newEngine.applyPreset(preset);

      expect(newEngine.hasMacros, isTrue);
      expect(newEngine.macroDefinitions.first.id, 'performance-swell');
      expect(newEngine.macroValue('performance-swell'), closeTo(1.0, 0.0001));
      expect(
        newEngine.oscillatorDetune,
        closeTo(detuneRange.denormalize(1.0), 0.0001),
      );
      expect(
        newEngine.distortionDrive,
        closeTo(driveRange.denormalize(0.8), 0.0001),
      );
  ...
  group('Parameter bridge integration', () {
    ...
    test('transport parameters clamp and propagate via the bridge', () async {
      final bridge = ParameterBridge.instance;
      final backend = BasicAudioBackend();
      final engine = AudioEngine(backend: backend, parameterBridge: bridge);
      ...
      await engine.setTransportTempo(178.0);
      await engine.setTransportRunning(0.0);
      await engine.setTransportTimeSignatureNumerator(9.0);
      await engine.setTransportTimeSignatureDenominator(5.0);
      await pumpEventQueue();

      expect(bridge.getParameter('tempo'), closeTo(178.0, 1e-9));
      expect(bridge.getParameter('transportPlay'), closeTo(0.0, 1e-9));
      expect(bridge.getParameter('tsNumerator'), closeTo(9.0, 1e-9));
      expect(bridge.getParameter('tsDenominator'), closeTo(5.0, 1e-9));

      bridge.updateParameter('bpm', 96.0, ParameterBridge.UpdateSource.ui);
      bridge.updateParameter('transportActive', 1.0, ParameterBridge.UpdateSource.ui);
      bridge.updateParameter(
      ...
      expect(engine.transportTempo, closeTo(96.0, 1e-9));
      expect(engine.transportRunning, isTrue);
      expect(engine.transportTimeSignatureNumerator, 7);
      expect(engine.transportTimeSignatureDenominator, 4);
      expect(
        backend.getParameter(SynthParameterId.transportTempo),
        closeTo(96.0, 1e-9),
      );
      expect(
        backend.getParameter(SynthParameterId.transportRunning),
        closeTo(1.0, 1e-9),
      );
  ...
  group('Tempo transport controls', () {
    test('applyTempoTransport updates backend and exposes state', () async {
      final backend = BasicAudioBackend();
      final engine = AudioEngine(backend: backend);

      expect(await engine.initialize(), isTrue);

      final notifications = <TempoTransportSettings>[];
      engine.addListener(() => notifications.add(engine.tempoTransport));

      const settings = TempoTransportSettings(
        bpm: 142.0,
        running: false,
      ...
      await engine.applyTempoTransport(settings);

      expect(engine.transportTempo, closeTo(142.0, 1e-9));
      expect(engine.transportRunning, isFalse);
      expect(engine.transportTimeSignatureNumerator, 3);
      expect(engine.transportTimeSignatureDenominator, 8);
      expect(engine.transportPositionBeats, closeTo(24.5, 1e-9));
      expect(
        backend.getParameter(SynthParameterId.transportTempo),
        closeTo(142.0, 1e-9),
      );
      ...
      expect(
        backend.getParameter(SynthParameterId.transportTimeSignatureNumerator),
        closeTo(3.0, 1e-9),
      );
      expect(
        backend.getParameter(SynthParameterId.transportTimeSignatureDenominator),
        closeTo(8.0, 1e-9),
      );
  ...
  group('Arpeggiator controls', () {
    test('applyArpeggiatorSettings updates backend and notifies once', () async {
      final backend = BasicAudioBackend();
      final engine = AudioEngine(backend: backend);

      expect(await engine.initialize(), isTrue);

      final notifications = <ArpeggiatorSettings>[];
      engine.addListener(() => notifications.add(engine.arpeggiatorSettings));

      const settings = ArpeggiatorSettings(
        enabled: true,
        rate: 5.5,
        gate: 0.4,
        octaveSpan: 3,
        mode: ArpeggiatorMode.random,
        pattern: ArpeggiatorPattern.majorTriad,
        swing: 0.15,
        latch: true,
        tempoSync: false,
        division: ArpeggiatorDivision.quarter,
      );

      await engine.applyArpeggiatorSettings(settings);

      expect(engine.arpeggiatorEnabled, isTrue);
      expect(engine.arpeggiatorRate, closeTo(5.5, 1e-9));
      expect(engine.arpeggiatorGate, closeTo(0.4, 1e-9));
      expect(engine.arpeggiatorOctaves, 3);
      expect(engine.arpeggiatorMode, ArpeggiatorMode.random);
      expect(engine.arpeggiatorPattern, ArpeggiatorPattern.majorTriad);
      expect(engine.arpeggiatorSwing, closeTo(0.15, 1e-9));
      expect(engine.arpeggiatorLatch, isTrue);
      expect(engine.arpeggiatorTempoSync, isFalse);
      expect(engine.arpeggiatorDivision, ArpeggiatorDivision.quarter);
      expect(
        backend.getParameter(SynthParameterId.arpeggiatorMode),
        closeTo(ArpeggiatorMode.random.value.toDouble(), 1e-9),
      );
      expect(
        backend.getParameter(SynthParameterId.arpeggiatorPattern),
        closeTo(ArpeggiatorPattern.majorTriad.value.toDouble(), 1e-9),
      );
      expect(notifications.length, 1);
    ...
    test('bridge aliases update arpeggiator state', () async {
      final bridge = ParameterBridge.instance;
      final backend = BasicAudioBackend();
      final engine = AudioEngine(backend: backend, parameterBridge: bridge);
      ...
      bridge.updateParameter('arpEnabled', 1.0, ParameterBridge.UpdateSource.ui);
      bridge.updateParameter('arp.rate', 14.0, ParameterBridge.UpdateSource.ui);
      bridge.updateParameter('arp_gate', 0.42, ParameterBridge.UpdateSource.ui);
      bridge.updateParameter('arpeggiator.range', 4.0, ParameterBridge.UpdateSource.ui);
      bridge.updateParameter('arpMode', 2.0, ParameterBridge.UpdateSource.ui);
      bridge.updateParameter('arpeggiatorShape', 3.0, ParameterBridge.UpdateSource.ui);
      bridge.updateParameter('arpSwing', 0.27, ParameterBridge.UpdateSource.ui);
      bridge.updateParameter('arpeggiatorHold', 1.0, ParameterBridge.UpdateSource.ui);

      await pumpEventQueue();

      expect(engine.arpeggiatorEnabled, isTrue);
      expect(engine.arpeggiatorRate, closeTo(14.0, 1e-9));
      expect(engine.arpeggiatorGate, closeTo(0.42, 1e-9));
      expect(engine.arpeggiatorOctaves, 4);
      expect(engine.arpeggiatorMode, ArpeggiatorMode.upDown);
      expect(engine.arpeggiatorPattern, ArpeggiatorPattern.octaves);
      expect(engine.arpeggiatorSwing, closeTo(0.27, 1e-9));
      expect(engine.arpeggiatorLatch, isTrue);
      expect(
        backend.getParameter(SynthParameterId.arpeggiatorLatch),
        closeTo(1.0, 1e-9),
      );
  ...
  group('Preset management', () {
    test('capturePreset snapshots current state and aliases', () async {
      ...
      await engine.setArpeggiatorEnabled(1.0);
      await engine.setArpeggiatorRate(9.5);
      await engine.setArpeggiatorGate(0.48);
      await engine.setArpeggiatorOctaves(3.0);
      await engine.setArpeggiatorMode(3.0);
      await engine.setArpeggiatorPattern(2.0);
      await engine.setArpeggiatorSwing(0.3);
      await engine.setArpeggiatorLatch(0.5);
      await engine.setArpeggiatorTempoSync(1.0);
      await engine.setArpeggiatorDivision(
          ArpeggiatorDivision.eighthTriplet.value.toDouble());
      await engine.setTransportTempo(134.0);
      await engine.setTransportRunning(0.0);
      await engine.setTransportTimeSignatureNumerator(7.0);
      await engine.setTransportTimeSignatureDenominator(8.0);
      ...
      expect(preset.parameters['arpeggiatorEnabled'], closeTo(1.0, 1e-9));
      expect(preset.parameters['arpeggiatorActive'], closeTo(1.0, 1e-9));
      expect(preset.parameters['arpEnabled'], closeTo(1.0, 1e-9));
      expect(preset.parameters['arpeggiatorRate'], closeTo(9.5, 1e-9));
      expect(preset.parameters['arpRate'], closeTo(9.5, 1e-9));
      expect(preset.parameters['arpeggiatorGate'], closeTo(0.48, 1e-9));
      expect(preset.parameters['arpGate'], closeTo(0.48, 1e-9));
      expect(preset.parameters['arpeggiatorOctaves'], closeTo(3.0, 1e-9));
      expect(preset.parameters['arpOctaves'], closeTo(3.0, 1e-9));
      expect(preset.parameters['arpeggiatorMode'], closeTo(3.0, 1e-9));
      expect(preset.parameters['arpMode'], closeTo(3.0, 1e-9));
      expect(preset.parameters['arpeggiatorPattern'], closeTo(2.0, 1e-9));
      expect(preset.parameters['arpPattern'], closeTo(2.0, 1e-9));
      expect(preset.parameters['arpeggiatorSwing'], closeTo(0.3, 1e-9));
      expect(preset.parameters['arpSwing'], closeTo(0.3, 1e-9));
      expect(preset.parameters['arpeggiatorLatch'], closeTo(0.5, 1e-9));
      expect(preset.parameters['arpLatch'], closeTo(0.5, 1e-9));
      expect(preset.parameters['arpeggiatorTempoSync'], closeTo(1.0, 1e-9));
      expect(preset.parameters['arpeggiatorSync'], closeTo(1.0, 1e-9));
      expect(
        preset.parameters['arpeggiatorDivision'],
        closeTo(ArpeggiatorDivision.eighthTriplet.value.toDouble(), 1e-9),
      );
      expect(
        preset.parameters['arpDivision'],
        closeTo(ArpeggiatorDivision.eighthTriplet.value.toDouble(), 1e-9),
      );
      expect(preset.parameters['transportTempo'], closeTo(134.0, 1e-9));
      expect(preset.parameters['tempo'], closeTo(134.0, 1e-9));
      expect(preset.parameters['transportRunning'], closeTo(0.0, 1e-9));
      expect(preset.parameters['transportPlay'], closeTo(0.0, 1e-9));
      expect(
        preset.parameters['transportTimeSignatureNumerator'],
        closeTo(7.0, 1e-9),
      );
      expect(preset.parameters['tsNumerator'], closeTo(7.0, 1e-9));
      expect(
        preset.parameters['transportTimeSignatureDenominator'],
        closeTo(8.0, 1e-9),
      );
      expect(preset.parameters['tsDenominator'], closeTo(8.0, 1e-9));
      expect(
    ...
    test('applyPreset updates backend, bridge and active preset snapshot', () async {
      ...
      final preset = SynthPreset(
        ...
        parameters: const <String, double>{
          ...
          'arpeggiatorRate': 7.5,
          'arpeggiatorGate': 0.44,
          'arpeggiatorOctaves': 2.0,
          'arpeggiatorMode': 1.0,
          'arpeggiatorPattern': 4.0,
          'arpeggiatorSwing': 0.18,
          'arpeggiatorLatch': 1.0,
        },
      );
      ...
      expect(bridge.getParameter('sustain'), closeTo(1.0, 1e-9));
      expect(engine.arpeggiatorEnabled, isTrue);
      expect(engine.arpeggiatorRate, closeTo(7.5, 1e-9));
      expect(engine.arpeggiatorGate, closeTo(0.44, 1e-9));
      expect(engine.arpeggiatorOctaves, 2);
      expect(engine.arpeggiatorMode, ArpeggiatorMode.down);
      expect(engine.arpeggiatorPattern, ArpeggiatorPattern.fifths);
      expect(engine.arpeggiatorSwing, closeTo(0.18, 1e-9));
      expect(engine.arpeggiatorLatch, isTrue);
      expect(bridge.getParameter('arpEnabled'), closeTo(1.0, 1e-9));
      expect(bridge.getParameter('arpRate'), closeTo(7.5, 1e-9));
      expect(bridge.getParameter('arpGate'), closeTo(0.44, 1e-9));
      expect(bridge.getParameter('arpOctaves'), closeTo(2.0, 1e-9));
      expect(bridge.getParameter('arpMode'), closeTo(1.0, 1e-9));
      expect(bridge.getParameter('arpPattern'), closeTo(4.0, 1e-9));
      expect(bridge.getParameter('arpSwing'), closeTo(0.18, 1e-9));
      expect(bridge.getParameter('arpLatch'), closeTo(1.0, 1e-9));
      final visualizer = engine.getVisualizerData();
      ...
      expect(visualizer['arpeggiatorRate'], closeTo(7.5, 1e-9));
      expect(visualizer['arpeggiatorLatch'], closeTo(1.0, 1e-9));
    ...
    test('loadPreset parses nested alias-heavy preset data', () async {
      ...
      expect(engine.arpeggiatorEnabled, isTrue);
      expect(engine.arpeggiatorRate, closeTo(11.0, 1e-9));
      expect(engine.arpeggiatorGate, closeTo(0.35, 1e-9));
      expect(engine.arpeggiatorOctaves, 3);
      expect(engine.arpeggiatorMode, ArpeggiatorMode.down);
      expect(engine.arpeggiatorPattern, ArpeggiatorPattern.fifths);
      expect(engine.arpeggiatorSwing, closeTo(0.2, 1e-9));
      expect(engine.arpeggiatorLatch, isTrue);
      ...
      expect(engine.activePreset?.parameters['arpeggiatorRate'], closeTo(11.0, 1e-9));
      expect(engine.activePreset?.parameters['arpPattern'], closeTo(4.0, 1e-9));
    ...
    test('exposes modulation metadata and aggregates depth', () async {
      final backend = BasicAudioBackend();
      final engine = AudioEngine(backend: backend);
      ...
      final sourceTotals = engine.modulationDepthBySource;
      final destinationTotals = engine.modulationDepthByDestination;

      expect(sourceTotals['lfo1'], closeTo(0.5, 1e-9));
    ...
    test('loadSetlistEntry wires context and applies preset', () async {
      final backend = BasicAudioBackend();
      final engine = AudioEngine(backend: backend);
      ...
      final setlistLibrary = PresetSetlistLibrary.instance;
      setlistLibrary.register(
        const PresetSetlist(
          id: 'showcase',
          name: 'Showcase',
          entries: [
      ...
      final loaded = await engine.loadSetlistEntry(
        setlistId: 'showcase',
        entryId: 'slot-1',
      );

      expect(loaded, isTrue);
      expect(engine.activeSetlist?.id, 'showcase');
      expect(engine.activeSetlistEntry?.label, 'Glow Pad');
      expect(engine.activePreset?.metadata.id, 'factory-glow-pad');
    ...
    test('manual preset loads clear setlist context', () async {
      final backend = BasicAudioBackend();
      final engine = AudioEngine(backend: backend);
      ...
      expect(engine.activeSetlistEntry, isNotNull);

      final manualLoad = await engine.loadPresetById('factory-laser-pluck');
      expect(manualLoad, isTrue);
      expect(engine.activeSetlistEntry, isNull);

      engine.dispose();
      backend.dispose();
    });

    test('clearSetlistContext resets context without touching preset', () async {
      final backend = BasicAudioBackend();
      final engine = AudioEngine(backend: backend);
      ...
      engine.clearSetlistContext();

      expect(engine.activeSetlist, isNull);
      expect(engine.activeSetlistEntry, isNull);
      expect(engine.activePreset?.metadata.id, 'factory-glow-pad');
  ...
  group('AudioEngine MIDI bindings', () {
    test('handleMidiControlChange maps controller values using bindings',
        () async {
      final backend = BasicAudioBackend();
      final manager = MidiBindingManager(registry: ParameterRegistry.instance);
      manager.clear();
      final engine = AudioEngine(
        backend: backend,
      ...
      manager.registerBinding(
        const MidiBinding(controller: 74, parameterId: 'cutoff'),
      );

      final first = await engine.handleMidiControlChange(74, 127);
      expect(first, isTrue);
      expect(
        engine.filterCutoff,
        closeTo(
          ParameterRegistry.instance
              .descriptorFor('filterCutoff')!
              .range
              .max,
          0.001,
        ),
      ...
      final second = await engine.handleMidiControlChange(74, 0);
      ...
      final missing = await engine.handleMidiControlChange(12, 90);
  ...
  group('AudioEngine step sequencer integration', () {
    test('sequencer advances trigger backend notes', () async {
      final backend = BasicAudioBackend();
      final engine = AudioEngine(backend: backend);

      expect(await engine.initialize(), isTrue);

      await engine.setTransportRunning(0, notify: false);

      final pattern = StepSequencerPattern(
      ...
      engine.updateStepSequencerPattern(pattern);
      ...
      expect(engine.activeNotes, contains(62));
      expect(engine.activeNotes.length, 1);
      expect(backend.activeVoiceCount, 1);
      expect(notifications, isNotEmpty);

      engine.stepSequencer.stop();
      engine.dispose();
      backend.dispose();
    });

    test('transport controls drive step sequencer state', () async {
      final backend = BasicAudioBackend();
      final engine = AudioEngine(backend: backend);

      expect(await engine.initialize(), isTrue);

      expect(engine.stepSequencer.isRunning, isTrue);

      await engine.setTransportRunning(0, notify: false);
      expect(engine.stepSequencer.isRunning, isFalse);

      await engine.setTransportTempo(90, notify: false);
      expect(engine.stepSequencer.bpm, closeTo(90, 1e-9));

      await engine.setTransportRunning(1, notify: false);
      expect(engine.stepSequencer.isRunning, isTrue);
  ...
  group('AudioEngine performance recording', () {
    test('captures note, parameter and preset events', () async {
      final backend = BasicAudioBackend();
      final engine = AudioEngine(backend: backend);
      final clock = _RecorderClock();
      ...
      clock.advance(const Duration(milliseconds: 5));
      await engine.applyPreset(preset, notify: false);

      clock.advance(const Duration(milliseconds: 5));
      final recording = recorder.stop();

      final noteOnEvent = recording.events
          .firstWhere((event) => event.type == PerformanceEventType.noteOn);
      expect(noteOnEvent.payload['note'], 60);
      expect(noteOnEvent.payload['velocity'], closeTo(0.75, 1e-9));

      final parameterEvent = recording.events.firstWhere(
        (event) =>
            event.type == PerformanceEventType.parameter &&
            event.payload['parameter'] == 'masterVolume',
      );
      expect(parameterEvent.payload['value'], closeTo(0.5, 1e-9));

      final presetEvent = recording.events
          .lastWhere((event) => event.type == PerformanceEventType.preset);
      expect(presetEvent.payload['presetId'], 'recorder-test');
