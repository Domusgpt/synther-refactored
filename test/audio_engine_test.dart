...
void main() {
  ...
  group('Parameter bridge integration', () {
    ...
    test('pitch wheel updates backend, bridge and visualiser', () async {
      final bridge = ParameterBridge.instance;
      final backend = BasicAudioBackend();
      final engine = AudioEngine(backend: backend, parameterBridge: bridge);
      ...
      await engine.setPitchBendRange(12.0);
      await engine.setPitchWheelValue(0.5);
      await pumpEventQueue();

      final metrics = engine.getVisualizerData();
      expect(metrics['pitchWheelValue'], closeTo(0.5, 1e-9));
      expect(metrics['pitchBendSemitones'], closeTo(6.0, 1e-9));
      expect(bridge.getParameter('pitchWheelNormalized'), closeTo(0.5, 1e-9));
      expect(bridge.getParameter('pitchBendAmount'), closeTo(0.5, 1e-9));

      final backendMetrics = backend.getVisualizerData();
      expect(backendMetrics['pitchBendSemitones'], closeTo(6.0, 1e-9));

      await engine.setPitchBendRange(4.0);
      await pumpEventQueue();

      final clamped = engine.getVisualizerData();
      expect(clamped['pitchBendSemitones'], closeTo(2.0, 1e-9));
      expect(clamped['pitchWheelValue'], closeTo(0.5, 1e-9));
  ...
  group('MIDI routing', () {
    test('default bindings respond to MIDI messages', () async {
      final bridge = ParameterBridge.instance;
      final backend = BasicAudioBackend();
      final engine = AudioEngine(backend: backend, parameterBridge: bridge);
      ...
      engine.midiRouter.handleMessage(<int>[0x90, 60, 100]);
      await pumpEventQueue();
      expect(engine.activeVoiceCount, 1);

      engine.midiRouter.handleMessage(<int>[0x90, 60, 0]);
      await pumpEventQueue();
      expect(engine.activeVoiceCount, 0);

      engine.midiRouter.handleMessage(<int>[0xB0, 74, 127]);
      await pumpEventQueue();
      expect(engine.filterCutoff, closeTo(20000, 1e-6));
      expect(bridge.getParameter('filterCutoff'), closeTo(20000, 1e-9));

      engine.midiRouter.handleMessage(<int>[0xE0, 0x00, 0x7F]);
      await pumpEventQueue();
      final data = engine.getVisualizerData();
      expect(data['pitchWheelValue'], closeTo(1.0, 1e-9));
