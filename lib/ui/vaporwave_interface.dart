...
import 'setlist_practice_panel.dart';
...
class _VaporwaveInterfaceState extends State<VaporwaveInterface>
        ...
        child: Padding(
          ...
          child: Column(
            ...
            children: [
              ...
              _buildStatusText(
                'PRACTICE',
                _resolvePracticeStatus(audioEngine),
              ),
              _buildStatusText(
              ...
              Row(
                children: [
                  ...
                  Expanded(
                    child: FilledButton.tonal(
                      style: FilledButton.styleFrom(
                        backgroundColor: const Color(0x3326FFB2),
                        foregroundColor: Colors.white,
                        padding: const EdgeInsets.symmetric(vertical: 10),
                        textStyle: const TextStyle(
                          fontWeight: FontWeight.w600,
                      ...
                      onPressed: () => _openSetlistPracticePanel(audioEngine),
  ...
  void _openSetlistPracticePanel(AudioEngine audioEngine) {
    showModalBottomSheet<void>(
      context: context,
      isScrollControlled: true,
      backgroundColor: Colors.transparent,
      builder: (context) {
        return ChangeNotifierProvider.value(
          value: audioEngine,
          child: const SetlistPracticePanel(),
        );
      },
    );
  }
  ...
  String _resolvePracticeStatus(AudioEngine audioEngine) {
    final snapshot = audioEngine.setlistPracticeSnapshot;
    if (snapshot == null || snapshot.totalEntries == 0) {
      return 'IDLE';
    }
    final currentIndex = snapshot.currentIndex;
    final total = snapshot.totalEntries;
    if (currentIndex < 0) {
      return '$total SLOT${total == 1 ? '' : 'S'} READY';
    }
    final label = snapshot.current?.entry.label ?? snapshot.current?.entry.id;
    if (label == null) {
      return 'SLOT ${currentIndex + 1} / $total';
    }
    return 'SLOT ${currentIndex + 1} / $total · ${label.toUpperCase()}';
  }
