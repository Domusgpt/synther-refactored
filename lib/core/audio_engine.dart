...
class AudioEngine extends ChangeNotifier {
  AudioEngine({
    ...
    _parameterSetters =
      ...
      'pitchWheelValue': (value, {notify = true}) =>
          setPitchWheelValue(value, notify: notify),
  ...
  double _pitchWheelValue =
      _parameterRegistry.defaultValue('pitchWheelValue') ?? 0.0;
  ...
  double get pitchWheelValue => _pitchWheelValue;
  SynthPreset? get activePreset => _activePreset;
  MidiRouter get midiRouter => _midiRouter ??= _createMidiRouter();
  ...
  Future<void> setPitchBendRange(double value, {bool notify = true}) async {
    ...
    _backend.setPitchBend(_pitchWheelValue * _pitchBendRange);
  }

  Future<void> setPitchWheelValue(double value, {bool notify = true}) async {
    handlePitchWheel(value, notify: notify);
  }

  MidiRouter _createMidiRouter() {
    final router = MidiRouter(
      onNoteOn: (note, velocity) => noteOn(note, velocity),
      onNoteOff: noteOff,
      onParameter: (name, value) => _handleMidiParameter(name, value),
      onPitchBend: (normalized) => handlePitchWheel(normalized),
    );

    router
      ..bindControllerToParameter(controller: 74, parameter: 'filterCutoff')
      ..bindControllerToParameter(controller: 71, parameter: 'filterResonance')
      ..bindControllerToParameter(controller: 1, parameter: 'lfoDepth');
  ...
  void handlePitchWheel(double normalized, {bool notify = true}) {
    final descriptor = _parameterRegistry.descriptorFor('pitchWheelValue');
    final range = descriptor?.range ??
        const ParameterRange(min: -1, max: 1, defaultValue: 0);
    final clamped = range.clamp(normalized);

    if ((clamped - _pitchWheelValue).abs() < 0.00001) {
      return;
    }

    _pitchWheelValue = clamped;
    final semitoneOffset = _pitchWheelValue * _pitchBendRange;
    _backend.setPitchBend(semitoneOffset);
    _emitParameterToBridge('pitchWheelValue', _pitchWheelValue);
  ...
  Map<String, double> getVisualizerData() {
    ...
    data['pitchWheelValue'] = _pitchWheelValue;
    data['pitchBendSemitones'] = _pitchWheelValue * _pitchBendRange;
  ...
  Map<String, double> _collectState({bool includeAliases = true}) {
    final values = <String, double>{
      ...
      'pitchWheelValue': _pitchWheelValue,
  ...
  void _handleMidiParameter(String name, double value) {
    final canonical = _canonicalParameterNameFor(name);
    final setter = _parameterSetters[canonical];
    if (setter == null) {
      return;
    ...
    final result = setter(value, notify: true);
