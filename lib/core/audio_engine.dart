...
class AudioEngine extends ChangeNotifier {
  AudioEngine({
    ...
    _parameterSetters = <String, Future<void> Function(double, {bool notify})>{
      ...
      'lfoRate': (value, {notify = true}) => setLfoRate(value, notify: notify),
  ...
  double _attackTime = _parameterRegistry.defaultValue('attackTime') ?? 0.02;
  double _decayTime = _parameterRegistry.defaultValue('decayTime') ?? 0.2;
  double _sustainLevel = _parameterRegistry.defaultValue('sustainLevel') ?? 0.7;
  double _releaseTime = _parameterRegistry.defaultValue('releaseTime') ?? 0.4;
  ...
  double _chorusRate = _parameterRegistry.defaultValue('chorusRate') ?? 1.2;
  double _chorusDepth = _parameterRegistry.defaultValue('chorusDepth') ?? 0.35;
  ...
  double _expression = _parameterRegistry.defaultValue('expression') ?? 1.0;
  double _sustainPedal = _parameterRegistry.defaultValue('sustainPedal') ?? 0.0;
  ...
  double _granularPan = _parameterRegistry.defaultValue('granularPan') ?? 0.0;
  ...
  double _transportTimeSignatureDenominator =
      _parameterRegistry.defaultValue('transportTimeSignatureDenominator') ??
      4.0;
  ...
  SetlistPracticeOptions _setlistPracticeOptions =
      const SetlistPracticeOptions();
  SetlistPracticeController? _setlistPracticeController;
  ...
  int get arpeggiatorOctaves => _arpeggiatorOctaves.round().clamp(1, 4).toInt();
  ArpeggiatorMode get arpeggiatorMode =>
      ArpeggiatorMode.values[_arpeggiatorMode
          .clamp(0, ArpeggiatorMode.values.length - 1)
          .toInt()];
  ...
  /// Returns modulation suggestions filtered by optional categories.
  ...
  Map<String, double> get modulationDepthByDestination {
    ...
    totals.forEach((rawKey, value) {
      final canonical = _parameterRegistry.canonicalName(rawKey) ?? rawKey;
  ...
  SetlistPracticeOptions get setlistPracticeOptions => _setlistPracticeOptions;
  SetlistPracticeSnapshot? get setlistPracticeSnapshot =>
      _setlistPracticeController?.snapshot();
  UnmodifiableListView<SetlistPracticeSegment> get setlistPracticeTimeline =>
      _setlistPracticeController?.timeline ?? _emptyPracticeTimeline;
  ...
  Future<void> setGranularGrainDuration(
    double value, {
    bool notify = true,
  }) async {
    await _updateParameter(
  ...
  Future<void> setGranularPositionVariation(
    double value, {
    bool notify = true,
  }) async {
    await _updateParameter(
      value: value,
      range: _parameterRegistry
          .descriptorFor('granularPositionVariation')!
          .range,
  ...
  Future<void> setGranularPitchVariation(
    double value, {
    bool notify = true,
  }) async {
    await _updateParameter(
      value: value,
      range: _parameterRegistry.descriptorFor('granularPitchVariation')!.range,
  ...
  Future<void> setGranularDurationVariation(
    double value, {
    bool notify = true,
  }) async {
    await _updateParameter(
      value: value,
      range: _parameterRegistry
          .descriptorFor('granularDurationVariation')!
          .range,
  ...
  Future<void> setGranularPanVariation(
    double value, {
    bool notify = true,
  }) async {
    await _updateParameter(
      value: value,
      range: _parameterRegistry.descriptorFor('granularPanVariation')!.range,
  ...
  Future<void> setGranularWindowType(double value, {bool notify = true}) async {
  ...
  Future<void> setArpeggiatorTempoSync(
    double value, {
    bool notify = true,
  }) async {
    await _updateParameter(
  ...
  Future<void> setArpeggiatorDivision(
    double value, {
    bool notify = true,
  }) async {
  ...
  Future<void> applyArpeggiatorSettings(
    ...
    await setArpeggiatorPattern(
      settings.pattern.value.toDouble(),
      notify: false,
    );
    await setArpeggiatorSwing(settings.swing, notify: false);
    await setArpeggiatorLatch(settings.latch ? 1.0 : 0.0, notify: false);
    await setArpeggiatorTempoSync(
      settings.tempoSync ? 1.0 : 0.0,
      notify: false,
    );
    await setArpeggiatorDivision(
      settings.division.value.toDouble(),
      notify: false,
    );

    final changed =
        previousEnabled != _arpeggiatorEnabled ||
  ...
  Future<void> setTransportTempo(double value, {bool notify = true}) async {
    ...
    _updatePracticeTempoState(notify: notify);
  }
  ...
  Future<void> setTransportTimeSignatureNumerator(
    double value, {
    bool notify = true,
  }) async {
    final descriptor = _parameterRegistry.descriptorFor(
  ...
  Future<void> setTransportTimeSignatureDenominator(
    double value, {
    bool notify = true,
  }) async {
    final descriptor = _parameterRegistry.descriptorFor(
  ...
  Future<void> applyTempoTransport(
    TempoTransportSettings settings, {
    bool notify = true,
  }) async {
    ...
    _updatePracticeTempoState(notify: notify && changed);
  }
  ...
  bool removeModulationRoute(
    String source,
    String destination, {
    bool notify = true,
  }) {
    final changed = _removeModulationRoute(
      source,
      destination,
      emitToBridge: true,
    );
  ...
  void noteOn(int note, [double velocity = 1.0]) {
    ...
    _backend.noteOn(allocation.voice.voiceId, note, normalisedVelocity);
    notifyListeners();
  }
  ...
  Map<String, double> getVisualizerData() {
    ...
    data['granularGrainRate'] = _granularActive >= 0.5
        ? _granularGrainRate
        : 0.0;
    ...
    data['granularMotion'] =
        (_granularPositionVariation +
            _granularPitchVariation +
    ...
    data['transportTimeSignatureNumerator'] = _transportTimeSignatureNumerator;
    ...
    for (final entry in destinationTotals.entries) {
      data['modDestination.${entry.key}'] = entry.value
          .clamp(0.0, 1.0)
          .toDouble();
    }
  ...
  void clearSetlistContext({bool notify = true}) {
    ...
    _setlistPracticeController = null;
    if (notify) {
      notifyListeners();
    }
  }

  void configureSetlistPractice({
    ...
    final updated = _setlistPracticeOptions.copyWith(
    ...
    if (updated == _setlistPracticeOptions) {
      return;
    }

    _setlistPracticeOptions = updated;
    if (_activeSetlist != null) {
      _ensurePracticeController(notify: false);
      _setlistPracticeController?.updateOptions(
        updated,
        currentEntryId: _activeSetlistEntry?.id,
      );
      _setlistPracticeController?.updateTempo(_currentTempoSettings());
    }
  ...
  void resetSetlistPractice({bool notify = true}) {
    if (_activeSetlist == null) {
      return;
    }
    _ensurePracticeController(notify: false);
    _setlistPracticeController?.reset(entryId: _activeSetlistEntry?.id);
    _setlistPracticeController?.updateTempo(_currentTempoSettings());
    if (notify) {
      notifyListeners();
    }
  ...
  Future<bool> advanceSetlistPractice({
    bool loadPreset = true,
    bool notify = true,
  }) async {
    final controller = _setlistPracticeController;
    final setlist = _activeSetlist;
    if (controller == null || setlist == null) {
      return false;
    }
  ...
  Future<bool> retreatSetlistPractice({
    bool loadPreset = true,
    bool notify = true,
  }) async {
    final controller = _setlistPracticeController;
  ...
  Future<bool> loadSetlistEntry({
    ...
    await applyPreset(preset, notify: notify, preserveSetlistContext: true);

    _activeSetlist = setlist;
    _activeSetlistEntry = entry;

    _ensurePracticeController(entryId: entry.id, notify: false);

    if (notify &&
        (previousSetlistId != setlist.id || previousEntryId != entry.id)) {
      notifyListeners();
    }
  ...
  Future<void> applyPreset(
    ...
    final matrixChanged = _replaceModulationMatrix(
    ...
    if (notify &&
        (changed || matrixChanged || previousPresetId != preset.metadata.id)) {
      notifyListeners();
    }
  }
  ...
  TempoTransportSettings _currentTempoSettings() {
    return TempoTransportSettings(
      bpm: _transportTempo,
      running: _transportRunning >= 0.5,
      timeSignatureNumerator: _transportTimeSignatureNumerator.round(),
      timeSignatureDenominator: _transportTimeSignatureDenominator.round(),
      positionBeats: _transportPositionBeats,
    );
  }

  void _ensurePracticeController({String? entryId, bool notify = false}) {
    final setlist = _activeSetlist;
    if (setlist == null) {
      if (_setlistPracticeController != null) {
        _setlistPracticeController = null;
        if (notify) {
          notifyListeners();
        }
    ...
    final tempo = _currentTempoSettings();
    final controller = _setlistPracticeController;
    if (controller == null) {
      _setlistPracticeController = SetlistPracticeController(
        setlist: setlist,
        tempo: tempo,
        options: _setlistPracticeOptions,
        currentEntryId: entryId ?? _activeSetlistEntry?.id,
      );
      if (notify) {
        notifyListeners();
      }
    ...
    controller.updateSetlist(
      setlist,
      currentEntryId: entryId ?? _activeSetlistEntry?.id,
    );
    controller.updateOptions(_setlistPracticeOptions);
    controller.updateTempo(tempo);

    final targetEntryId = entryId ?? _activeSetlistEntry?.id;
    if (targetEntryId != null) {
      controller.jumpToEntry(targetEntryId);
    }
  ...
  void _updatePracticeTempoState({bool notify = false}) {
    final controller = _setlistPracticeController;
    if (controller == null) {
      return;
    }
    controller.updateTempo(_currentTempoSettings());
    if (notify) {
      notifyListeners();
    }
  }
  ...
  Map<String, double> _collectState({bool includeAliases = true}) {
    ...
    final practiceController = _setlistPracticeController;
    if (practiceController != null) {
      values['practiceLoopCount'] = practiceController.loopCount.toDouble();
      values['practiceEntries'] = practiceController.totalEntries.toDouble();
      final index = practiceController.currentIndex;
      if (index != null) {
        values['practiceCurrentIndex'] = index.toDouble();
      }
      values['practiceCountInBeats'] = _setlistPracticeOptions.countInBeats;
      values['practiceCueBeats'] = _setlistPracticeOptions.defaultCueBeats;
      values['practiceEntryBeats'] = _setlistPracticeOptions.defaultEntryBeats;
    }
  ...
  List<ModulationRoute>? _extractModulationRoutes(Map<String, dynamic> preset) {
    ...
    if (preset['modulationMatrix'] is Map) {
      found = true;
      mapCandidates.add(
        (preset['modulationMatrix'] as Map).cast<dynamic, dynamic>(),
      );
    }
  ...
  static Map<String, double> _flattenPresetValues(Map<String, dynamic> preset) {
  ...
  static double? _coerceToParameterValue(String path, dynamic value) {
    ...
    if (value is String) {
      ...
      if (_matchesNormalizedKey(normalizedKey, const <String>[
        ...
        if (_matchesNormalizedKey(sanitizedValue, const <String>[
          'true',
          'on',
          'yes',
        ])) {
      ...
      if (_matchesNormalizedKey(normalizedKey, const <String>[
        ...
        for (final division in ArpeggiatorDivision.values) {
          ...
          if (divisionKey == sanitizedValue ||
              sanitizedValue ==
                  ParameterRegistry.normalizeKey(divisionLabel(division)) ||
  ...
  static bool _matchesNormalizedKey(
    String key,
  ...
  void _pushStateToBackend() {
    ...
    _backend.setParameter(
      SynthParameterId.channelAftertouch,
      _channelAftertouch,
    );
    ...
    _backend.setParameter(
      SynthParameterId.granularGrainRate,
      _granularGrainRate,
    );
    _backend.setParameter(
      SynthParameterId.granularGrainDuration,
      _granularGrainDuration,
    );
    ...
    _backend.setParameter(
      SynthParameterId.granularAmplitude,
      _granularAmplitude,
    );
    _backend.setParameter(
      SynthParameterId.granularPositionVariation,
      _granularPositionVariation,
    );
    _backend.setParameter(
      SynthParameterId.granularPitchVariation,
      _granularPitchVariation,
    );
    _backend.setParameter(
      SynthParameterId.granularDurationVariation,
      _granularDurationVariation,
    );
    _backend.setParameter(SynthParameterId.granularPan, _granularPan);
    _backend.setParameter(
      SynthParameterId.granularPanVariation,
      _granularPanVariation,
    );
    _backend.setParameter(
      SynthParameterId.granularWindowType,
      _granularWindowType,
    );
    _backend.setParameter(
      SynthParameterId.wavetablePosition,
      _wavetablePosition,
    );
    _backend.setParameter(SynthParameterId.microphoneVolume, _microphoneVolume);
    _backend.setParameter(
      SynthParameterId.arpeggiatorEnabled,
      _arpeggiatorEnabled,
    );
    _backend.setParameter(SynthParameterId.arpeggiatorRate, _arpeggiatorRate);
    _backend.setParameter(SynthParameterId.arpeggiatorGate, _arpeggiatorGate);
    _backend.setParameter(
      SynthParameterId.arpeggiatorOctaves,
      _arpeggiatorOctaves,
    );
    _backend.setParameter(SynthParameterId.arpeggiatorMode, _arpeggiatorMode);
    _backend.setParameter(
      SynthParameterId.arpeggiatorPattern,
      _arpeggiatorPattern,
    );
    _backend.setParameter(SynthParameterId.arpeggiatorSwing, _arpeggiatorSwing);
    _backend.setParameter(SynthParameterId.arpeggiatorLatch, _arpeggiatorLatch);
    _backend.setParameter(
      SynthParameterId.arpeggiatorTempoSync,
      _arpeggiatorTempoSync,
    );
    _backend.setParameter(
      SynthParameterId.arpeggiatorDivision,
      _arpeggiatorDivision,
    );
    _backend.setParameter(SynthParameterId.transportTempo, _transportTempo);
    _backend.setParameter(SynthParameterId.transportRunning, _transportRunning);
    _backend.setParameter(
      SynthParameterId.transportTimeSignatureNumerator,
  ...
  bool _replaceModulationMatrix(
    Iterable<ModulationRoute> routes, {
    bool emitToBridge = true,
  }) {
    final previous = {
      for (final route in _modulationMatrix.routes) route.key: route,
    };
  ...
  void _emitModulationRouteToBridge(ModulationRoute route) {
    ...
    final key = ModulationMatrixCodec.encodeBridgeKey(
      route.source,
      route.destination,
    );
  ...
  Future<void> _handleBridgeUpdate(String name, double value) async {
    ...
    if (modulationRoute != null) {
      final changed = _applyModulationRoute(
        modulationRoute,
        emitToBridge: false,
      );
      if (changed) {
