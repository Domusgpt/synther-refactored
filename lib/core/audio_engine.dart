...
class AudioEngine extends ChangeNotifier {
  ...
  /// Register a listener that will receive high-level engine events.
  void addEventListener(AudioEngineEventListener listener) {
    _eventListeners.add(listener);
  }

  /// Stop delivering events to a listener previously added via [addEventListener].
  void removeEventListener(AudioEngineEventListener listener) {
  ...
  final Set<AudioEngineEventListener> _eventListeners =
  ...
  Future<bool> loadSetlistEntry({
    ...
    if (previousSetlistId != setlist.id || previousEntryId != entry.id) {
      _emitEvent(
        AudioEngineEvent.setlistChanged(
  ...
  Future<void> applyPreset(
    ...
    if (changed ||
      ...
      _emitEvent(AudioEngineEvent.presetApplied(preset.metadata.id));
    }
  ...
  Future<void> loadPreset(Map<String, dynamic> preset) async {
    ...
    if (changed || matrixChanged) {
      _emitEvent(AudioEngineEvent.presetApplied(_activePreset?.metadata.id));
    }
  ...
  void _emitParameterToBridge(String name, double value) {
    _emitEvent(AudioEngineEvent.parameterChanged(name, value));
  ...
  void _emitEvent(AudioEngineEvent event) {
    if (_eventListeners.isEmpty) {
      return;
    }

    final listeners = List<AudioEngineEventListener>.from(_eventListeners);
