...
class AudioEngine extends ChangeNotifier {
  AudioEngine({
    ...
    MidiBindingManager? midiBindingManager,
    MacroController? macroController,
    StepSequencer? stepSequencer,
    PresetSetlistPersistence? setlistPersistence,
        ...
        _midiBindingManager =
            midiBindingManager ?? MidiBindingManager.instance,
        _macroController = macroController ?? MacroController(),
        _stepSequencer = stepSequencer ?? StepSequencer(),
        _ownsStepSequencer = stepSequencer == null,
        _setlistPersistence = setlistPersistence {
    _instanceCount++;
    ...
    _hookStepSequencer();
    _stepSequencer.setBpm(_transportTempo);
    if (_transportRunning >= 0.5) {
      _stepSequencer.start();
    }
  ...
  final MidiBindingManager _midiBindingManager;
  final MacroController _macroController;
  StepSequencer _stepSequencer;
  bool _ownsStepSequencer;
  StreamSubscription<StepSequencerPlaybackEvent>? _stepSequencerSubscription;
  PresetSetlistPersistence? _setlistPersistence;
  final Map<int, _SequencerVoice> _sequencerVoices =
      <int, _SequencerVoice>{};
  PerformanceRecorder? _performanceRecorder;
  ...
  PresetSetlist? _activeSetlist;
  PresetSetlistEntry? _activeSetlistEntry;

  bool get isInitialized => _isInitialized;
  String? get lastError => _lastError;

  UnmodifiableListView<MacroDefinition> get macroDefinitions =>
      _macroController.definitions;
  List<MacroSnapshot> get macroSnapshots => _macroController.snapshot();
  bool get hasMacros => !_macroController.isEmpty;
  double macroValue(String id) => _macroController.valueFor(id);
  StepSequencer get stepSequencer => _stepSequencer;
  ...
  PresetSetlist? get activeSetlist => _activeSetlist;
  PresetSetlistEntry? get activeSetlistEntry => _activeSetlistEntry;
  List<SynthPreset> get availablePresets =>
      AudioPresetLibrary.instance.allPresets();
  List<PresetSetlist> get availableSetlists {
    final library = PresetSetlistLibrary.instance;
    library.ensureBuiltInSetlists();
    return library.allSetlists;
  }
  PresetSetlistPersistence? get setlistPersistence => _setlistPersistence;
  int get maxPolyphony => _voiceAllocator.maxVoices;

  PerformanceRecorder? get performanceRecorder => _performanceRecorder;

  void attachPerformanceRecorder(PerformanceRecorder? recorder,
      {bool emitSnapshot = true}) {
    _performanceRecorder = recorder;
    if (recorder != null && emitSnapshot && recorder.isRecording) {
      recorder.recordSnapshot(
        parameters: _collectState(includeAliases: true),
        presetId: _activePreset?.metadata.id,
        presetName: _activePreset?.metadata.name,
        setlistId: _activeSetlist?.id,
        setlistEntryId: _activeSetlistEntry?.id,
      );
      recorder.recordTransportState(
        tempo: _transportTempo,
        running: transportRunning,
        numerator: transportTimeSignatureNumerator,
        denominator: transportTimeSignatureDenominator,
        positionBeats: _transportPositionBeats,
      );
    }
  }
  ...
  Future<void> registerMacro(MacroDefinition definition, {bool notify = true})
      async {
    final changed = _macroController.register(definition);
    if (changed && notify) {
      notifyListeners();
    }
  }

  Future<void> registerMacros(
    Iterable<MacroDefinition> definitions, {
    bool notify = true,
  }) async {
    final changed = _macroController.registerAll(definitions);
    if (changed && notify) {
      notifyListeners();
    }
  }

  Future<bool> setMacroValue(String id, double value, {bool notify = true}) async {
    if (!_macroController.contains(id)) {
      return false;
    }

    final updates = _macroController.setValue(id, value);
    final definition =
        _macroController.definitions.firstWhere((macro) => macro.id == id);
    _performanceRecorder?.recordMacroValue(id, value, updates);
    final changed = await _applyMacroUpdates(updates);
    final shouldNotify =
        notify && (changed || updates.isNotEmpty || definition.assignments.isEmpty);
    if (shouldNotify) {
      notifyListeners();
    }
    return changed || updates.isNotEmpty || definition.assignments.isEmpty;
  }

  Future<void> resetMacroValues({bool notify = true}) async {
    if (!hasMacros) {
      return;
    }

    var changed = false;
    for (final definition in _macroController.definitions) {
      final result = await setMacroValue(
        definition.id,
    ...
    if (notify && (changed || _macroController.definitions.isNotEmpty)) {
      notifyListeners();
    }
  }

  void clearMacros({bool notify = true}) {
    final changed = _macroController.clear();
    if (changed && notify) {
      notifyListeners();
    }
  }

  void attachSetlistPersistence(
    PresetSetlistPersistence persistence, {
    bool load = false,
  }) {
    _setlistPersistence?.dispose();
    _setlistPersistence = persistence;
    if (load) {
      unawaited(persistence.load());
    }
  }
  ...
  Future<void> setTransportTempo(double value, {bool notify = true}) async {
    final previous = _transportTempo;
    await _updateParameter(
      value: value,
    ...
    if ((previous - _transportTempo).abs() >= 0.00001) {
      _stepSequencer.setBpm(_transportTempo);
      _recordTransportSnapshot();
    }
  }

  Future<void> setTransportRunning(double value, {bool notify = true}) async {
    final previous = _transportRunning;
    await _updateParameter(
      value: value,
    ...
    if ((previous - _transportRunning).abs() >= 0.00001) {
      if (_transportRunning >= 0.5) {
        _stepSequencer.start();
      } else {
        _stepSequencer.stop(resetPosition: false);
        _releaseSequencerVoices(reason: 'transportStopped');
      }
      _recordTransportSnapshot();
    }
  }
  ...
  Future<void> setTransportTimeSignatureNumerator(double value,
    ...
    _performanceRecorder?.recordParameterChange(
  ...
  void noteOn(int note, [double velocity = 1.0]) {
    ...
    if (allocation.hasStolenVoices) {
      for (final stolen in allocation.stolenVoices) {
        ...
        _performanceRecorder?.recordNoteOff(
          note: stolen.note,
          voiceId: stolen.voiceId,
    ...
    _performanceRecorder?.recordNoteOn(
      note: note,
      velocity: normalisedVelocity,
      voiceId: allocation.voice.voiceId,
    );
    notifyListeners();
  }
  ...
  void attachStepSequencer(
    StepSequencer sequencer, {
    bool takeOwnership = false,
    bool resetPosition = true,
  }) {
    if (identical(_stepSequencer, sequencer)) {
      _ownsStepSequencer = _ownsStepSequencer || takeOwnership;
      return;
    }

    final previous = _stepSequencer;
    final previouslyOwned = _ownsStepSequencer;

    _stepSequencer.stop(resetPosition: false);
    _releaseSequencerVoices(reason: 'sequencerSwap');
    unawaited(_stepSequencerSubscription?.cancel());
    _stepSequencerSubscription = null;

    _stepSequencer = sequencer;
    _ownsStepSequencer = takeOwnership;

    if (resetPosition) {
      sequencer.reset();
    } else {
      sequencer.stop(resetPosition: false, emitRelease: false);
    }

    sequencer.setBpm(_transportTempo);
    _hookStepSequencer(sequencer);

    if (_transportRunning >= 0.5) {
      sequencer.start(resetPosition: resetPosition);
    } else if (resetPosition) {
      sequencer.stop(resetPosition: true, emitRelease: false);
    }
  ...
  void updateStepSequencerPattern(
    StepSequencerPattern pattern, {
    bool resetPosition = true,
  }) {
    _stepSequencer.setPattern(pattern, resetPosition: resetPosition);
    notifyListeners();
  }
  ...
  Map<String, double> getVisualizerData() {
    ...
    data['sequencerStepIndex'] = _stepSequencer.currentStep.toDouble();
    data['sequencerRunning'] = _stepSequencer.isRunning ? 1.0 : 0.0;
  ...
  SynthPreset capturePreset({
    ...
    final preset = SynthPreset(
      ...
      modulationRoutes: _modulationMatrix.routes,
      macros: _macroController.snapshot(),
    );
  ...
  void clearSetlistContext({bool notify = true}) {
    if (_activeSetlist == null && _activeSetlistEntry == null) {
      return;
    }
    _activeSetlist = null;
    _activeSetlistEntry = null;
    _performanceRecorder?.recordSetlistChange(cleared: true);
    if (notify) {
      notifyListeners();
    }
  }
  ...
  Future<bool> loadSetlistEntry({
    required String setlistId,
    required String entryId,
    bool notify = true,
  }) async {
    final setlistLibrary = PresetSetlistLibrary.instance;
    setlistLibrary.ensureBuiltInSetlists();
    final setlist = setlistLibrary.findById(setlistId);
    if (setlist == null) {
      return false;
    }

    final entry = setlist.entryById(entryId);
    if (entry == null) {
      return false;
    }
    ...
    final preset = presetLibrary.findById(entry.presetId);
    if (preset == null) {
      return false;
    }

    final previousSetlistId = _activeSetlist?.id;
    final previousEntryId = _activeSetlistEntry?.id;

    await applyPreset(
      preset,
      notify: notify,
      preserveSetlistContext: true,
    );

    _activeSetlist = setlist;
    _activeSetlistEntry = entry;

    if (previousSetlistId != setlist.id || previousEntryId != entry.id) {
      _performanceRecorder?.recordSetlistChange(
    ...
    if (notify && (previousSetlistId != setlist.id || previousEntryId != entry.id)) {
      notifyListeners();
    }
  ...
  Future<void> applyPreset(
    SynthPreset preset, {
    bool notify = true,
    bool preserveSetlistContext = false,
  }) async {
    ...
    final macrosChanged = await _applyMacroSnapshots(
      preset.macros.toList(),
      notify: false,
    );
    ...
    _syncSetlistContext(
      _activePreset?.metadata.id,
      preserve: preserveSetlistContext,
    );

    if (changed || matrixChanged || macrosChanged ||
        previousPresetId != preset.metadata.id) {
      _performanceRecorder?.recordPresetChange(
        presetId: preset.metadata.id,
        presetName: preset.metadata.name,
        snapshot: Map<String, double>.from(snapshot),
      );
    }

    if (notify &&
        (changed || matrixChanged || macrosChanged ||
            previousPresetId != preset.metadata.id)) {
      notifyListeners();
    }
  }
  ...
  Future<void> loadPreset(Map<String, dynamic> preset) async {
    ...
    final capturedParameters = _collectState(includeAliases: true);
    ...
    _syncSetlistContext(metadata.id);
    _performanceRecorder?.recordPresetChange(
      presetId: metadata.id,
      presetName: metadata.name,
      snapshot: Map<String, double>.from(capturedParameters),
    );
  ...
  void _syncSetlistContext(String? presetId, {bool preserve = false}) {
    if (preserve) {
      return;
    }

    final entry = _activeSetlistEntry;
    if (entry == null) {
      return;
    }

    if (presetId == null || entry.presetId != presetId) {
      _activeSetlist = null;
      _activeSetlistEntry = null;
      _performanceRecorder?.recordSetlistChange(cleared: true);
    }
  ...
  Map<String, double> _collectState({bool includeAliases = true}) {
    final values = <String, double>{
      ...
      'sequencerStepIndex': _stepSequencer.currentStep.toDouble(),
      'sequencerRunning': _stepSequencer.isRunning ? 1.0 : 0.0,
    };
  ...
  Future<bool> _applyMacroUpdates(Map<String, double> updates) async {
    if (updates.isEmpty) {
      return false;
    }

    final before = _collectState(includeAliases: false);
    var applied = false;
    for (final entry in updates.entries) {
      final setter = _parameterSetters[entry.key];
      if (setter == null) {
        continue;
      }
      applied = true;
    ...
    final after = _collectState(includeAliases: false);
    final changed = updates.keys.any((key) {
      final previous = before[key];
      final next = after[key];
      if (previous == null || next == null) {
        return false;
      }
      return (previous - next).abs() >= 0.00001;
    });

    return changed;
  }

  Future<bool> _applyMacroSnapshots(
    List<MacroSnapshot> snapshots, {
    bool notify = true,
  }) async {
    final previousIds =
        _macroController.definitions.map((macro) => macro.id).toList();
    final updates =
        _macroController.replaceWithSnapshots(List<MacroSnapshot>.from(snapshots));
    final changedValues = await _applyMacroUpdates(updates);
    final currentIds =
        _macroController.definitions.map((macro) => macro.id).toList();
    final definitionsChanged = !listEquals(previousIds, currentIds);
    final changed = changedValues || definitionsChanged;
  ...
  void _recordTransportSnapshot() {
    final recorder = _performanceRecorder;
    if (recorder == null) {
      return;
    }
  ...
  void _hookStepSequencer([StepSequencer? sequencer]) {
    final target = sequencer ?? _stepSequencer;
    unawaited(_stepSequencerSubscription?.cancel());
    _stepSequencerSubscription =
        target.playbackStream.listen(_handleStepSequencerEvent);
  }

  void _handleStepSequencerEvent(StepSequencerPlaybackEvent event) {
    switch (event.type) {
      case StepSequencerPlaybackEventType.step:
        notifyListeners();
        return;
      case StepSequencerPlaybackEventType.noteOn:
        if (!_backend.isInitialized) {
          return;
        }
        final note = event.note;
        final instanceId = event.noteInstanceId;
        if (note == null || instanceId == null) {
          return;
        }
        final velocity = (event.velocity ?? 1.0).clamp(0.0, 1.0).toDouble();
        final allocation = _voiceAllocator.allocate(note, velocity);
        if (allocation.hasStolenVoices) {
          for (final stolen in allocation.stolenVoices) {
            _removeSequencerVoiceByVoiceId(stolen.voiceId);
            if (_backend.isInitialized) {
              _backend.noteOff(stolen.voiceId);
            }
            _performanceRecorder?.recordNoteOff(
        ...
        _backend.noteOn(allocation.voice.voiceId, note, velocity);
        _performanceRecorder?.recordNoteOn(
          note: note,
          velocity: velocity,
          voiceId: allocation.voice.voiceId,
        ...
        _sequencerVoices[instanceId] = _SequencerVoice(
          note: note,
          voiceId: allocation.voice.voiceId,
          velocity: velocity,
        );
        notifyListeners();
        return;
      case StepSequencerPlaybackEventType.noteOff:
        if (!_backend.isInitialized) {
          final instanceId = event.noteInstanceId;
          if (instanceId != null) {
            _sequencerVoices.remove(instanceId);
          }
          return;
        }
        final instanceId = event.noteInstanceId;
        ...
        final voice = _sequencerVoices.remove(instanceId);
        if (voice == null) {
          return;
        }
        final released = _voiceAllocator.releaseVoice(voice.voiceId);
        _backend.noteOff(voice.voiceId);
        _performanceRecorder?.recordNoteOff(
          note: released?.note ?? voice.note,
          voiceId: voice.voiceId,
          velocity: released?.velocity ?? voice.velocity,
          reason: 'sequencer',
        );
        notifyListeners();
        return;
  ...
  void _releaseSequencerVoices({String reason = 'sequencer'}) {
    if (_sequencerVoices.isEmpty) {
      return;
    }

    final pending = _sequencerVoices.values.toList(growable: false);
    _sequencerVoices.clear();

    for (final voice in pending) {
      final released = _voiceAllocator.releaseVoice(voice.voiceId);
      if (_backend.isInitialized) {
        _backend.noteOff(voice.voiceId);
      }
  ...
  void _removeSequencerVoiceByVoiceId(int voiceId) {
    final toRemove = <int>[];
    _sequencerVoices.forEach((key, voice) {
      if (voice.voiceId == voiceId) {
        toRemove.add(key);
      }
    });
    for (final key in toRemove) {
      _sequencerVoices.remove(key);
    }
  }
  ...
  void dispose() {
    ...
    _stepSequencer.stop(resetPosition: false);
    _releaseSequencerVoices(reason: 'dispose');
    unawaited(_stepSequencerSubscription?.cancel());
    _stepSequencerSubscription = null;
    if (_ownsStepSequencer) {
      _stepSequencer.dispose();
    }
    final releasedVoices = _voiceAllocator.reset();
    _macroController.clear();
  ...
  /// Exposes the shared MIDI binding manager for configuration.
  MidiBindingManager get midiBindings => _midiBindingManager;

  /// Applies MIDI control change data to the engine using the registered
  /// bindings. Returns `true` when any parameter was updated.
  Future<bool> handleMidiControlChange(
    int controller,
    int value, {
    bool notify = true,
  }) async {
    final updates = _midiBindingManager.processControlChange(controller, value);
    if (updates.isEmpty) {
      return false;
    }

    final changed = await _applyPresetValues(updates);
    if (!changed) {
      return false;
    }
...
class _SequencerVoice {
  const _SequencerVoice({
    required this.note,
