...
class BasicAudioBackend implements AudioBackend {
  ...
  void setParameter(int parameterId, double value) {
    if (parameterId == SynthParameterId.pitchBendRange) {
      final range = clampValue<double>(value, 1.0, 24.0);
      _parameters[parameterId] = range;
      _pitchBendSemitones = clampValue<double>(
        _pitchBendSemitones,
        -range,
        range,
      );
  ...
  Map<String, double> getVisualizerData() {
    ...
    final pitchWheelValue = pitchBendRange == 0
        ? 0.0
        : clampValue<double>(
            _pitchBendSemitones / pitchBendRange,
            -1.0,
            1.0,
          );
    ...
    return {
      ...
      'pitchBendSemitones': _pitchBendSemitones,
      'pitchWheelValue': pitchWheelValue,
    };
  }
  ...
  double _midiToFrequency(int note) {
    return 440.0 * math.pow(2, ((note - 69) + _pitchBendSemitones) / 12.0);
  }
}
