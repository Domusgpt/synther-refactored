...
- Introduced an engine event bus (`lib/core/audio_engine_events.dart`) with
  performance recorder integration (`lib/core/performance_recorder.dart`) so QA
  sessions can capture parameter changes, note events, preset swaps, and setlist
  transitions with timestamped metadata.
...
- **Macro UI:** Surface macros inside the holographic interface (panels, HUD
  indicators, MIDI learn shortcuts) so performers can tweak macro sweeps without
  diving into preset editors.
...
- **Performance playback:** With the new recorder capturing engine events, add a
  transport-aligned playback/looping surface so recorded takes can be rehearsed
  or exported to automation lanes.
- **Event filtering/export:** Provide CLI tooling or in-app filters to convert
  captured event logs into human-readable reports, CSV/JSON exports, or DAW
  automation clips.
