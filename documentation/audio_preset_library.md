...
- `macro1`..`macro4` – four freely assignable macro controls (aliases such as
  `macroOne`, `macroA`, etc.) captured with presets, mirrored to the bridge, and
  surfaced via the `macroAverage` and enhanced `performanceEnergy` visualiser
  metrics so live rigs can lean on macro pads or knobs without extra glue.
...
expressive mod wheel, aftertouch, expression pedal, sustain pedal, and macro
values so factory sounds translate directly to live contexts and automation
workflows.
