...
When you are ready to produce shareable artifacts, run
`tool/package_release.sh` to generate both the release APK and a zipped web
bundle in one shot. The script copies the outputs into `build/packages/` with
timestamped names and accepts `--no-web`, `--no-android`, or `--mode profile`
flags when you want more control.
...
that anyone can launch from their browser. When you are ready to ship a build,
follow the [Release Checklist](release_checklist.md) for a detailed verification
and packaging walkthrough.
