...
## Turn 9 – Suggestion favourites & bundles
- Added stable suggestion identifiers, an expanded curated catalogue, and metadata helpers for favourites, recents, and workflow bundles.
- Exposed favourite/recency/bundle APIs from `AudioEngine` plus bundle application helpers that track usage alongside modulation matrix changes.
- Refreshed the modulation matrix panel with favourite toggles, recent chips, bundle cards, and preview integrations while updating docs and tests.

## Turn 10 – Usage insights & trending workflows
- Tracked suggestion usage counts and popularity tags inside `AudioEngine`, exposing trending picks, top tags, and reset helpers alongside history clearing.
- Surfaced popular tag chips, trending suggestion badges with usage counts, and quick reset/clear affordances in the modulation matrix panel.
- Expanded the curated catalogue with macro and ambience-focused bundles, updated documentation, and extended unit coverage for the new insights APIs.
