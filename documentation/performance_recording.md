...
## Holographic Recorder Panel

Open the **RECORDER** button from the status HUD to manage the recorder inside
the app. The [`PerformanceRecorderPanel`](../lib/ui/performance_recorder_panel.dart)
wires up the active `AudioEngine` so you can:

- Start fresh or resume a capture without leaving the vaporwave interface.
- Monitor live event counts, durations, and recent payloads while playing.
- Copy the latest JSON snapshot to the clipboard or review it inline before
  sharing with QA.
- Clear stale sessions before starting a new take.

The panel auto-instantiates a recorder if one is not already attached, making it
easy for testers to capture sessions without writing code.
...
- **`noteOn`** – payload: `note`, `velocity`, `voiceId`, optional `source`.
  Emitted whenever the engine allocates a voice. Sequencer-driven phrases tag
  events with `source: 'sequencer'`. Stolen voices are released and logged
  immediately before the new note triggers.
- **`noteOff`** – payload: `note`, `voiceId`, optional `velocity` and `reason`.
  Fired for manual releases, sequencer stops (`reason: 'sequencer'`), voice
  stealing, polyphony trims, and engine disposal.
