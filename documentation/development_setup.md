...
- **ImageMagick / Preview tooling** (optional) to inspect the PNGs produced by
  `tool/capture_ui_snapshot.sh` when reviewing UI regressions.
- **MIDI monitoring utilities** like MIDI Monitor (macOS), MIDI-OX (Windows), or
  `aseqdump`/`kmidimon` (Linux) for validating controller data while exercising
  the MIDI binding manager.
...
tool/capture_ui_snapshot.sh  # Capture widget test driven screenshots + metadata
tool/package_release.sh      # Generate release APK + web bundle into build/packages/
```
